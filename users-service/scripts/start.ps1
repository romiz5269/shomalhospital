# Users service — بیمارستان شمال — port 5002
...
Write-Host "`n[1/4] Docker: Postgres users (:5435)" -ForegroundColor Cyan
Ensure-InfraContainer -Name "hospital-postgres-users" -ComposeUpArgs @("compose","up","-d","postgres-users") | Out-Null

Write-Host "`n[2/4] Wait healthy..." -ForegroundColor Cyan
$pg = "unknown"
for ($i = 1; $i -le 30; $i++) {
    $ErrorActionPreference = "Continue"
    $pg = docker inspect hospital-postgres-users --format "{{.State.Health.Status}}" 2>$null
    $ErrorActionPreference = "Stop"
    if ($pg -eq "healthy") { break }
    Start-Sleep 2
}
Write-Host "Postgres users: $pg" -ForegroundColor Green
...
Write-Host "`n[4/4] Start Users on :$Port" -ForegroundColor Green
Stop-PortListener $Port
Write-Host "  Docs:  http://127.0.0.1:$Port/docs" -ForegroundColor Yellow
Write-Host "  Site:  http://127.0.0.1:8080/api/v1/users/... (via gateway + JWT)`n" -ForegroundColor Yellow
