# Production start Blog — port 5005 (behind gateway as pages service)
...
if (-not (Test-Path $pub)) {
    ...
    if (Test-Path $fallback) {
        ...
        Write-Host "Copied auth public.pem -> blog-service/keys/public.pem" -ForegroundColor Yellow
    } else {
...
Write-Host "Blog PRODUCTION gunicorn w=$Workers :5005" -ForegroundColor Green
